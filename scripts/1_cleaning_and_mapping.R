##Reading raw data
raw_data = read_excel(paste0(getwd(),'/data inputs/raw_data.xlsx')) %>% mutate(record_id = 1:n())

##Reading mapping matrix
mapping_matrix = read_excel(paste0(getwd(),'/data inputs/mapping_matrix.xlsx'))

###Assigning site-specific variables to standard variables; and also checking if numeric entries were done (if so, these are converted to letters)
##Reorganizing the mapping matrix to extract content for automated analysis: clearly defining what the variables are and assigning labels to the various response categories
...
# Converting datasets with integers into letters (in case the data are integers - i.e. 1, 2, 3 instead of A, B, C):
...
# Edit checks: The checks done are based on 2021 core questionnaire. For reference, the checks are commented out at the end of this script. 
...
# Set out-of-range values to missing (erroneous multi-responses would also be removed)
...
too_many_similar_responses = function(response, threshold = 15) {
  ...
  # Find all matches of repeated characters
  matches = str_match_all(response, "(.)\\1*")[[1]]
  # Get the maximum length of repeated characters
  max_repeat <- max(nchar(matches[,1]), na.rm = TRUE)
  max_repeat >= threshold
}

# Apply the function to each row and summarize
raw_data = raw_data %>% mutate(many_similar = apply(.[,-1], 1, too_many_similar_responses))%>% dplyr::filter(many_similar==FALSE)
...
# updating excluded variables list to include the "many similar" variables
excl_variables = setdiff(names(raw_data), grep('q|height|weight',names(raw_data),v=T))
