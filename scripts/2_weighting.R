...
###Assigning psus
# sample_schools_with_psu_strata = prestratification_weights %>% left_join(final_set) %>% 
#                                  mutate(class_id = as.character(class_id),
#                                         psu = ifelse(scwgt==1,paste0(999999,school_id, class_id), school_id), psu = as.numeric(psu))%>% 
...
} else if(weighting_sex_only == TRUE)
{
  ...
  eval(parse(text=paste0('raw_data$post_adj_factor[raw_data$char_SAMPL_SEX=="',post_strat_adj[,2]$sex,'&',
                         '" & raw_data$category =="',post_strat_adj[,1]$category,'"] = ', post_strat_adj[,3]$post_adj, sep= '\n')))
}else if(no_post_strat_weighting == TRUE)
{
  raw_data$post_adj_factor = 1
} else{
...
###For Ghana:::::
# if(site_name == 'Ghana (Sekondi-Takoradi)' & survey_year =='2022')
# {
# raw_data$DE_GRADE[raw_data$DE_GRADE=='JHS 3'|raw_data$DE_GRADE=='SHS 3']=NA
# raw_data$DE_GRADE=factor(raw_data$DE_GRADE)
# raw_data$original_q_3[raw_data$original_q_3=='C'|raw_data$original_q_3=='F']=NA
# 
# } 
# if((site_name == 'UNRWA (Gaza)'| site_name == 'UNRWA (West Bank)'| site_name == 'UNRWA (Syria)') & survey_year =='2022')
# {
#   raw_data$DE_GRADE[raw_data$DE_GRADE=='Grade 10'|raw_data$DE_GRADE=='Grade 11'|raw_data$DE_GRADE=='Grade 12']=NA
#   raw_data$DE_GRADE=factor(raw_data$DE_GRADE)
#   raw_data$original_q3[raw_data$original_q3=='D'|raw_data$original_q3=='E'|raw_data$original_q3=='F']=NA
# } 
# if((site_name == 'UNRWA (Jordan)') & survey_year =='2022')
# {
#   raw_data$DE_GRADE[raw_data$DE_GRADE=='Grade 11'|raw_data$DE_GRADE=='Grade 12']=NA
#   raw_data$DE_GRADE=factor(raw_data$DE_GRADE)
#   raw_data$original_q3[raw_data$original_q3=='E'|raw_data$original_q3=='F']=NA
#   
# }else{}
