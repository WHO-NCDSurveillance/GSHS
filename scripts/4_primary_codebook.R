...
# if (language =='ENGLISH')
# {
#   eval(parse(text=paste0('data$',new_variables, '=factor(data$',new_variables,',levels=c("Yes","No"), labels =c("Yes","No")',')', sep='\n')))
# } else if (language =='FRENCH') {
#   eval(parse(text=paste0('data$',new_variables, '=factor(data$',new_variables,',levels=c("Yes","No"), labels =c("Oui","Non")',')', sep='\n')))
# }else if (language =='SPANISH') {
#   eval(parse(text=paste0('data$',new_variables, '=factor(data$',new_variables,',levels=c("Yes","No"), labels =c("Sí","No")',')', sep='\n')))
# } else if (language =='RUSSIAN') {
#   eval(parse(text=paste0('data$',new_variables, '=factor(data$',new_variables,',levels=c("Yes","No"), labels =c("Да","Нет")',')', sep='\n')))
# } else{}


# if (language == 'ENGLISH'){
#   colnames(final_output) = c('Standard\nVariable\nName','Country\nQuestionnaire\nNumber','Question\nCode and Label','Label','Unweighted\nFrequency','Weighted\nPercentage')
# } else if (language =='FRENCH'){  
#   colnames(final_output) = c('Nom standard\nde la variable','Numéro dans \nle questionnaire\ndu pays','Code de\nquestion \net réponses','Marqué','Fréquence \n non pondérée','Pourcentage\npondéré')
# }else if (language =='SPANISH'){  
#   colnames(final_output) = c('Nombre estándar\nde la variable','Número del \ncuestionario\ndel país','Código de \npregunta y \netiqueta','Etiqueta','Frecuencia \nno ponderada','Porcentaje\nponderado')
# }else if (language =='RUSSIAN'){  
#   colnames(final_output) = c('Стандартное имя\nпеременной','Номер анкеты \nдля страны','Код вопроса\n и метка','Этикетка','Невзвешенная\n частота','Взвешенный \nпроцент')
# } else{}
