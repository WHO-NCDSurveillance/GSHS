...
additional_text = c(survey_year,
                    ...
                    site_name,
                    paste0(paste0(tolower(sort(unique(factsheet_sections$sub_title))[-length(unique(factsheet_sections$sub_title))]), collapse = ', '),
                           eval(parse(text=lang_titles[22]))[1],tolower(sort(unique(factsheet_sections$sub_title))[length(unique(factsheet_sections$sub_title))])),
...
print(doc,target=paste0(getwd(),'/Batch Reports/',survey_year,' ' ,site_name,' GSHS Fact Sheet.docx')) 
